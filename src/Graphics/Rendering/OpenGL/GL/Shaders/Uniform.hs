...
makeUniformVar setter location = makeStateVar getter (setter location)
  where
    getter = allocaBytes maxUniformBufferSize $ \buf -> do
...
single var location =
  makeStateVar
    (do Vertex1 x <- get (var location); return x)
...
instance Uniform TextureUnit where
  uniform loc = makeStateVar getter setter
    where
      getter = allocaBytes (sizeOf (undefined :: GLint)) $ \buf -> do
...
instance MatrixComponent a => Uniform (GLmatrix a) where
  uniform loc@(UniformLocation ul) = makeStateVar getter setter
    where
      getter = withNewMatrix ColumnMajor $ getUniformWith getUniformv loc
  ...
  uniformv (UniformLocation ul) count buf =
    ...
    where
      elemType = undefined :: MatrixComponent c => Ptr (GLmatrix c) -> Ptr c
...
newtype UniformBlockIndex = UniformBlockIndex GLuint
  deriving (Eq, Show, Ord)

uniformBlockIndex :: Program -> String -> GettableStateVar UniformBlockIndex
uniformBlockIndex (Program program) name =
  makeGettableStateVar $
    fmap UniformBlockIndex $
      withGLstring name $
        glGetUniformBlockIndex program

uniformBlockBinding :: Program -> UniformBlockIndex -> SettableStateVar GLuint
uniformBlockBinding (Program program) (UniformBlockIndex idx) = makeSettableStateVar $ \bindingPoint -> glUniformBlockBinding program idx bindingPoint
